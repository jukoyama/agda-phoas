...
mutual
  data cpsterm𝑐[_,_] (var : cpstyp → Set) (cvar : conttyp → Set) :
                     cpstyp → Set where
    CPSRet : {τ₁ τ₂ : cpstyp} →
  ...
  data cpsvalue𝑐[_,_] (var : cpstyp → Set) (cvar : conttyp → Set) :
                      cpstyp → Set where
  ...
  data cpscont𝑐[_,_] (var : cpstyp → Set) (cvar : conttyp → Set) :
                     conttyp → Set where
    CPSKVar : {τ₁ τ₂ : cpstyp} →
              cvar (τ₁ ⇒ τ₂) →
              cpscont𝑐[ var , cvar ] (τ₁ ⇒ τ₂)
...
-- 値による代入規則
mutual
  data cpsSubstValV𝑐 {var : cpstyp → Set} {cvar : conttyp → Set} : {τ τ₁ : cpstyp} →
    ...
    sShift : {τ τ₁ τ₂ τ₃ τ₄ τ₅ : cpstyp} {v : cpsvalue𝑐[ var , cvar ] τ} →
             cpsSubstValV𝑐 (λ _ → CPSShift {τ₁ = τ₁} {τ₂} {τ₃} {τ₄} {τ₅}) v CPSShift
    sFun  : {τ′ τ₀ τ₁ τ₃ τ₄ : cpstyp} →
            {e  : var τ′ →  var τ₀ → cvar (τ₁ ⇒ τ₃) → cpsterm𝑐[ var , cvar ] τ₄} → 
  ...
  data cpsSubstV𝑐 {var : cpstyp → Set} {cvar : conttyp → Set} : {τ₁ τ₂ : cpstyp} →
                  (var τ₁ → cpsterm𝑐[ var , cvar ] τ₂) →
                  cpsvalue𝑐[ var , cvar ] τ₁ →
  ...
  data cpsSubstContV𝑐 {var : cpstyp → Set} {cvar : conttyp → Set} : {τ τ₁ τ₂ : cpstyp} →
    ...
    sKId   : {τ τ₁ : cpstyp} {v : cpsvalue𝑐[ var , cvar ] τ} →
...
-- 値と継続の代入規則
mutual
  data cpsSubst𝑐 {var : cpstyp → Set} {cvar : conttyp → Set} : {τ τ₁ α β : cpstyp} →
                 (var τ → cvar (α ⇒ β) → cpsterm𝑐[ var , cvar ] τ₁) →
                 cpsvalue𝑐[ var , cvar ] τ →
                 cpscont𝑐[ var , cvar ] (α ⇒ β) → 
                 cpsterm𝑐[ var , cvar ] τ₁ → Set where
    ...
    sRetE : {τ τ₁ τ₂ α β : cpstyp} →
            {k₁  : var τ → cvar (α ⇒ β) → cpscont𝑐[ var , cvar ] (τ₁ ⇒ τ₂)} →
            {e₂  : var τ → cvar (α ⇒ β) → cpsterm𝑐[ var , cvar ] τ₁} →
            {v   : cpsvalue𝑐[ var , cvar ] τ} →
            {c   : cpscont𝑐[ var , cvar ] (α ⇒ β)} → 
            {k₁′ : cpscont𝑐[ var , cvar ] (τ₁ ⇒ τ₂)} →
            {e₂′ : cpsterm𝑐[ var , cvar ] τ₁} →
  ...
  data cpsSubstCont𝑐 {var : cpstyp → Set} {cvar : conttyp → Set} : {τ τ₁ τ₂ α β : cpstyp} →
                     (var τ → cvar (α ⇒ β) →
                     cpscont𝑐[ var , cvar ] (τ₁ ⇒ τ₂)) →
                     cpsvalue𝑐[ var , cvar ] τ →
                     cpscont𝑐[ var , cvar ] (α ⇒ β) → 
                     cpscont𝑐[ var , cvar ] (τ₁ ⇒ τ₂) → Set where
    sKVar= : {τ α β : cpstyp} {v : cpsvalue𝑐[ var , cvar ] τ}
             {c : cpscont𝑐[ var , cvar ] (α ⇒ β)} →
             cpsSubstCont𝑐 (λ _ k → CPSKVar k) v c c
    sKVar≠ : {τ α β τ₁ τ₂ : cpstyp}
             {v : cpsvalue𝑐[ var , cvar ] τ}
             {c : cpscont𝑐[ var , cvar ] (α ⇒ β)} {k′ : cvar (τ₁ ⇒ τ₂)} →
             cpsSubstCont𝑐 (λ _ _ → CPSKVar k′) v c (CPSKVar k′)
    sKId   : {τ α β τ₁ : cpstyp} →
             {v : cpsvalue𝑐[ var , cvar ] τ} {c : cpscont𝑐[ var , cvar ] (α ⇒ β)} →
             cpsSubstCont𝑐 {τ₁ = τ₁} (λ _ _ → CPSId) v c CPSId
    sKFun  : {τ τ₁ τ₂ α β : cpstyp} →
             {e₁ : var τ → cvar (α ⇒ β) → var τ₁ → cpsterm𝑐[ var , cvar ] τ₂} → 
             {v  : cpsvalue𝑐[ var , cvar ] τ} → 
             {c  : cpscont𝑐[ var , cvar ] (α ⇒ β)} →
             {e₁′ : var τ₁ → cpsterm𝑐[ var , cvar ] τ₂} → 
...
data cpsReduce• {var : cpstyp → Set} {cvar : conttyp → Set} :
     ...
     βShift𝑐  : {τ₁ τ₃ τ₄ : cpstyp} →
                {w : cpsvalue𝑐[ var , cvar ] ((τ₃ ⇒[ τ₄ ⇒ τ₃ ]⇒ τ₃) ⇒[ τ₁ ⇒ τ₁ ]⇒ τ₄)} →
                {j : cpscont𝑐[ var , cvar ] (τ₃ ⇒ τ₄)} →
                cpsReduce• (CPSApp CPSShift w j)
                           (CPSApp w (CPSFun (λ x k → CPSRetE (CPSKVar k) (CPSRet j (CPSVar x)))) CPSId)

data cpsReduce𝑅 {var : cpstyp → Set} {cvar : conttyp → Set} :
                {τ₂ : cpstyp} →
                cpsterm𝑐[ var , cvar ] τ₂ →
                cpsvalue𝑐[ var , cvar ] τ₂ → Set where
     βReset𝑐 : {τ₁ : cpstyp} →
...
data cpsReduceV {var : cpstyp → Set} {cvar : conttyp → Set} :
                 {τ₁ : cpstyp} →
                 cpsvalue𝑐[ var , cvar ] τ₁ →
                 cpsvalue𝑐[ var , cvar ] τ₁ → Set where
...
data cpsReduceK {var : cpstyp → Set} {cvar : conttyp → Set} :
                {τ₁ τ₂ : cpstyp} →
                cpscont𝑐[ var , cvar ] (τ₁ ⇒ τ₂) →
                cpscont𝑐[ var , cvar ] (τ₁ ⇒ τ₂) → Set where
     ηLet𝑐 : {τ₁ τ₂ : cpstyp} →
